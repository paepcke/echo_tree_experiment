...
function ExperimentManager () {
	...
	// Ascii printable range? The following
	// returns single, one-char string. If the 
	// key code is non-printable, the string will be empty:
	charAsStr = String.fromCharCode(evt.which);
	
	// Backspace returns an empty string, but
	// we implement that one:
	if (charAsStr.length == 0) {
	    if  (evt.which === ASCII_BACKSPACE)
